...
export default class BubbleChart extends Component {
  ...
  renderChart() {
    ...
    const color = d3.scaleOrdinal().domain([0, 1]).range(["#5BA8F6","#4EEEB3", "#CCE5FF"]);;
  ...
  renderBubbles(width, nodes, color) {
    ...
    node.append("circle")
      ...
      .attr("r", function(d) { console.log('Append Circle:',d); return d.r - (d.r * .04); })
      .style("fill", function(d) { return d.data.color ? d.data.color : color(nodes.indexOf(d)); })
      .style("z-index", 1)
      .attr("title", 'Test Title')
      .on('mouseover', function(d) {
        console.log('Circle mouseover:', d)
        console.log('Circle mouseover - labele:', d.fromElement.__data__.label)
        const data_label = d.fromElement.__data__.label;

        // d3.select(this).attr("r", d.r * 1.04);
        // Show tooltip
        tooltip.text(data_label); return tooltip.style("visibility", "visible");
      })
      .on("mousemove", function(d){return tooltip.style("top", (d.pageY-10)+"px").style("left",(d.pageX+10)+"px");})
      .on('mouseout', function(d) {
        console.log('Circle mouseout:', d)
